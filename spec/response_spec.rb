...
describe Onelogin::Saml::Response do
  describe "decrypting assertions" do
    ...
    it "should find the right attributes from an encrypted assertion" do
      ...
      REXML::XPath.first(@response.document, "/samlp:Response/saml:Assertion").should_not be_nil
      REXML::XPath.first(@response.document, "/samlp:Response/saml:Assertion/ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestValue").text.should == "eMQal6uuWKMbUMbOwBfrFH90bzE="
    ...
    it "should not be able to decrypt without the proper key" do
      ...
      @response = Onelogin::Saml::Response.new(@xmlb64, @settings)
      REXML::XPath.first(@response.document, "/samlp:Response/saml:Assertion").should be_nil
