...
<EntityDescriptor xmlns="urn:oasis:names:tc:SAML:2.0:metadata" entityID="#{settings.issuer}">
  <SPSSODescriptor protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
    ...
    <AssertionConsumerService index="0" Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="#{settings.sp_assertion_consumer_url}"/>
  </SPSSODescriptor>
  <ContactPerson contactType="technical">
    <SurName>#{settings.tech_contact_name}</SurName>
    <EmailAddress>#{settings.tech_contact_email}</EmailAddress>
  </ContactPerson>
</EntityDescriptor>}
    end
  end
